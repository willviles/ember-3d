...
import { BoxGeometry, Matrix4, Mesh, MeshPhongMaterial, FlatShading } from 'three';
import color from '../config/colors';

const { get, computed: { reads }, getProperties, set } = Ember;
...
export default GroupObjectMixin.extend({
  ...
  cabin() {
  	...
  	const material = new MeshPhongMaterial({color: color.red, shading: FlatShading});
  ...
  engine() {
    const geometry = new BoxGeometry(20,50,50,1,1,1);
  	const material = new MeshPhongMaterial({color: color.white, shading: FlatShading});
  ...
  blades() {
    const geometry = new BoxGeometry(1,100,20,1,1,1);
  	const material = new MeshPhongMaterial({color: color.brownDark, shading: FlatShading});
  ...
  animate() {
    let { propeller, object } = getProperties(this, 'propeller', 'object');

    let loop = () => {

      let { mouseX, mouseY } = getProperties(this, 'mouseX', 'mouseY'),
